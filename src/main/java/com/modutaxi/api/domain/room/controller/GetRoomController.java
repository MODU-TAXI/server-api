...
public class GetRoomController {
    ...
    @Operation(summary = "지도, 리스트 통합 조회",
        description = "조회 위치 요청 반경의 원형 내 방의 정보를 리스트로 조회합니다.<br>**필수 파라미터**<br>&emsp;**searchLongitude**(경도)<br>&emsp;**searchLatitude**(위도)<br><br>**선택 파라미터**<br>&emsp;**spotId**(지정 거점id)<br>&emsp;**radius**(조회 반경) : 기본값(500m)<br>&emsp;**roomTags**(방 카테고리)<br>&emsp;**isImminent**(마감 임박 여부)<br>&emsp;**sortType**(정렬 기준)<br><br>**sortType**<br>&emsp;**NEW** : 1. 가장 최근에 생성된 순서로 정렬 2. 가장 가까운 순서로 정렬<br>&emsp;**DISTANCE** : 1. 가장 가까운 순서로 정렬 2. 출발시간이 가장 이른 순서로 정렬<br>&emsp;**ENDTIME** : 1. 출발시간이 가장 이른 순서로 정렬 2. 가장 가까운 순서로 정렬"
    )
    @GetMapping("/integration")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "방 목록 조회 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = SearchRoomIntegrationResponses.class))),
        ...
        @ApiResponse(responseCode = "400", description = "방 목록 조회 실패", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RoomErrorCode.class), examples = {
    ...
    public ResponseEntity<SearchRoomIntegrationResponses> getRoomIntegration(
        ...
        @Parameter(description = "검색지 경도") @RequestParam(value = "searchLongitude") Float searchLongitude,
        @Parameter(description = "검색지 위도") @RequestParam(value = "searchLatitude") Float searchLatitude,
        ...
        return ResponseEntity.ok(getRoomService.getRoomIntegration(spotId, roomTags, point, radius, isImminent, sortType));
    }
