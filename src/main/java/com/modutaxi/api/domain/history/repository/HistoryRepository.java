...
public interface HistoryRepository extends JpaRepository<History, Long> {
    ...
    List<History> findByMemberIdAndDepartureDate(@Param("memberId") Long memberId,
        @Param("year") int year, @Param("month") int month);

    @Query(
        "SELECT SUM(h.totalCharge) as accumulateTotalCharge, SUM(h.portionCharge) as accumulatePortionCharge "
            +
            "FROM History h WHERE h.member.id = :memberId " +
            "AND YEAR(h.room.departureTime) = :year AND MONTH(h.room.departureTime) = :month")
    Tuple findTotalChargeAndPortionChargeByMemberIdAndDepartureDate(
